...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("arches_rascolls", "0002_plugin"),
    ]

    create_searchable_values_table = """
        CREATE SEQUENCE IF NOT EXISTS public."arches_rascolls_searchable_values_id_seq"
            INCREMENT 1
            START 1
            MINVALUE 1
            MAXVALUE 2147483647
        ...
        CREATE TABLE IF NOT EXISTS public.arches_rascolls_searchable_values
        (
            id bigint NOT NULL DEFAULT nextval('arches_rascolls_searchable_values_id_seq'::regclass),
            tileid uuid,
        ...
        ALTER SEQUENCE public."arches_rascolls_searchable_values_id_seq"
            OWNED BY public."arches_rascolls_searchable_values".id;

        ALTER SEQUENCE public."arches_rascolls_searchable_values_id_seq"
            OWNER TO postgres;

        ALTER TABLE IF EXISTS public.arches_rascolls_searchable_values
            OWNER to postgres;

        ALTER TABLE IF EXISTS public.arches_rascolls_searchable_values 
            ADD COLUMN search_vector tsvector;

        CREATE INDEX arches_rascolls_searchable_values_idx 
            ON arches_rascolls_searchable_values USING GIN(search_vector);      
    """
    ...
    populate_searchable_values_table = """
        INSERT INTO arches_rascolls_searchable_values (tileid, resourceinstanceid, value)
        SELECT DISTINCT
            t.tileid,
        ...
        UPDATE public.arches_rascolls_searchable_values
            SET search_vector = setweight(to_tsvector('english', coalesce(value,'')), 'A');  
    """

    reverse_populate_searchable_values_table = """
        DELETE FROM arches_rascolls_searchable_values;
    """

    create_function_to_update_searchable_values = """
        CREATE OR REPLACE FUNCTION __arches_rascolls_update_searchable_values_for_tile(_tileid UUID)
        RETURNS VOID AS $$
        BEGIN
            DELETE FROM arches_rascolls_searchable_values WHERE arches_rascolls_searchable_values.tileid = _tileid;
            INSERT INTO arches_rascolls_searchable_values (tileid, resourceinstanceid, value)
            SELECT DISTINCT
                t.tileid,
                t.resourceinstanceid,
